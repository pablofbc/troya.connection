...
export const comunidadApi = async () => {
  try {
    const response = await axiosApi.get(
      `https://cc210ecba693.sn.mynetname.net:9091/comunidadAPI/api/UserOrganization/GetFormByCodUserByCodOrg?CodUser=AACA6001019D6&CodOrg=COD_MPI`
    );
    return response.data;
  } catch (error) {
...
export const editorApi = async (codUser: String, codOrg: String) => {
  try {
    ...
    // );
    const response = await axiosApi.get(
      `https://cc210ecba693.sn.mynetname.net:9091/FormEditionAPI/api/Personas/GetPersonas/CE_3133195_202410090517381738`
    );
    return response.data;
  } catch (error) {
...
export const conectarApi = async (username: string, password: string) => {
  try {
